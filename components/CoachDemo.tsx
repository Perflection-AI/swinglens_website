...
export const CoachDemo: React.FC = () => {
  ...
  return (
    <section id="demo" className="py-24 bg-paper relative overflow-hidden border-t border-ink/5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-24 items-start">
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-32">
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-light border border-green-light shadow-sm text-ink text-xs font-bold uppercase tracking-wider mb-6 font-mono">
              ...
              <div className="flex flex-wrap gap-2">
                {["Fix my slice", "Add 10 yards", "Stop 3-putting", "Better bunker shots"].map((q) => (
                  <button 
                    key={q}
                    onClick={() => setQuery(q)}
                    className="text-sm bg-white border border-ink/10 hover:border-green-light hover:bg-paper hover:text-ink text-subtle px-4 py-2 rounded-lg transition-all shadow-sm hover:shadow-card font-mono"
                  >
                    {q}
                  </button>
                ))}
            ...
            <div className="bg-paper rounded-[1.5rem] shadow-soft-xl border border-ink/10 overflow-hidden flex flex-col h-[600px] relative">
              ...
              <div className="bg-white/90 backdrop-blur-sm border-b border-ink/10 p-4 flex items-center justify-between absolute top-0 left-0 right-0 z-20">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-green-light flex items-center justify-center text-green shadow-sm border border-green-light">
              ...
              <div className="flex-1 overflow-y-auto p-6 pt-24 space-y-6 custom-scrollbar bg-ruled-paper">
                ...
                <div className="flex gap-4 max-w-[90%]">
                  <div className="w-8 h-8 rounded-lg bg-green-light flex-shrink-0 flex items-center justify-center text-white text-xs border border-green font-mono">
                    AI
                  </div>
                ...
                {(status === LoadingState.LOADING || status === LoadingState.SUCCESS) && (
                   <div className="flex gap-4 max-w-[90%] ml-auto flex-row-reverse animate-in slide-in-from-bottom-2 fade-in duration-300">
                    <div className="w-8 h-8 rounded-lg bg-green flex-shrink-0 flex items-center justify-center text-white">
                      <User className="w-4 h-4" />
                    </div>
                    {/* Darker green for user bubble for contrast, but cleaner */}
                    <div className="bg-green text-white p-5 rounded-xl rounded-tr-none shadow-card text-sm leading-relaxed">
                      {query}
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex gap-4 max-w-[90%] animate-in slide-in-from-bottom-2 fade-in duration-500">
                    ...
                    <div className="space-y-3 w-full">
                      <div className="bg-white border border-ink/10 p-6 rounded-xl rounded-tl-none shadow-card text-ink text-sm leading-relaxed relative">
                        ...
                        <span className="block font-bold text-green mb-2 text-xs uppercase tracking-wide pl-2 font-mono">Analysis</span>
                        <div className="pl-2">{response.advice}</div>
                      </div>
                      
                      {/* Drill Box: Fresh Green Background - Removed Lit Shadow */}
                      <div className="bg-green-light border border-green-light p-5 rounded-xl shadow-sm text-ink text-sm leading-relaxed">
                        <span className="flex items-center gap-2 font-bold text-ink mb-2 text-xs uppercase tracking-wide font-mono">
                          <Sparkles className="w-3 h-3 text-green" /> Recommended Drill
                        </span>
                        {response.drill}
                      </div>
                    </div>
              ...
              <div className="p-4 bg-white border-t border-ink/10 relative z-20">
                <form onSubmit={handleAnalyze} className="relative">
                  <input
                    ...
                    className="w-full bg-paper text-ink border border-ink/10 focus:border-green-light focus:ring-4 focus:ring-green-light rounded-xl py-3.5 pl-5 pr-12 text-sm font-medium transition-all outline-none placeholder:text-subtle/50 shadow-inner font-mono placeholder:font-sans"
                  ...
                  <button 
                    ...
                    className={`
                      ...
                      ${query.trim() ? 'bg-green text-white hover:bg-green shadow-sm transform hover:scale-105' : 'bg-green-light text-green-light cursor-not-allowed'}
                    `}
                  >
